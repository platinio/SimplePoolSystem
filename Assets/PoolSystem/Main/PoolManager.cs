...
namespace Platinio.PoolSystem
{
    ...
    public class PoolManager : Singleton<PoolManager>
    {
        ...
        private List<DelayUnspawn> delayUnspawnRequest = new List<DelayUnspawn>();
        #region UNITY_EVENTS
        private void Awake()
        {
            Platinio.CheckPoolConnections();
        ...
        private void LateUpdate()
        {
            ...
            for (int n = 0; n < delayUnspawnRequest.Count; n++)
            {
                delayUnspawnRequest[n].time -= Time.deltaTime;

                if (delayUnspawnRequest[n].time <= 0.0f)
                {
                    delayUnspawnRequest[n].go.Unspawn();
                    delayUnspawnRequest.RemoveAt( n );
                }
            }
        }
        #endregion
        ...
        {
            delayUnspawnRequest.Add( new DelayUnspawn() { go = go, time = t } );
        }
